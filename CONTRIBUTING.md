...
4. State the version of the `@typescript-nameof` packages you use (always show a reproduction of the bug on the latest version).
...
3. Ensure both `npm run lint` and `npm run verify` passes when run in the root directory.
...
4. Ensure both `npm run lint` and `npm run verify` passes when run in the root directory.
